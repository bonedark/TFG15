 ...
 * 
 * @param <T>
 *            The type of entity evolved by the evolution engine.
 * @author Daniel Dyer
 * @see CandidateFactory
 * @see FitnessEvaluator
 */
public abstract class AbstractEvolutionEngine<T extends Comparable<? super T>> implements EvolutionEngine<T> {
	...
	public List<EvaluatedCandidate<T>> evolvePopulation(int populationSize, int eliteCount,
		...
		PopulationData<T> data = EvolutionUtils.getPopulationData(evaluatedPopulation, fitnessEvaluator.isNatural(),
				eliteCount, currentGenerationIndex, startTime);
		...
		while (satisfiedConditions == null) {
			...
			EvolutionUtils.sortEvaluatedPopulation(evaluatedPopulation, fitnessEvaluator.isNatural());
			data = EvolutionUtils.getPopulationData(evaluatedPopulation, fitnessEvaluator.isNatural(), eliteCount,
	 ...
	 *            The population at the beginning of the process.
	 * @param eliteCount
	 *            The number of the fittest individuals that must be preserved.
	 * @param rng
	 ...
	 *            fitness score).
	 * @return The evaluated population (a list of candidates with attached
	 *         fitness scores).
	 */
	protected List<EvaluatedCandidate<T>> evaluatePopulation(List<T> population) {
		...
		if (singleThreaded) // Do fitness evaluations on the request thread.
		{
			for (T candidate : population) {
				evaluatedPopulation.add(new EvaluatedCandidate<T>(candidate, fitnessEvaluator.getFitness(candidate,
						population)));
			}
		} else {
			...
			try {
				...
				List<Future<EvaluatedCandidate<T>>> results = new LinkedList<Future<EvaluatedCandidate<T>>>();
				...
				for (int i = population.size(); i-- > 0;) {
					...
					results.add(
							...
							getSharedWorker().submit(
									new FitnessEvalutationTask<T>(fitnessEvaluator, candidate, unmodifiablePopulation)));
				}
	 ...
	 * many as there are available cores/processors). Use this method to force
	 * evaluation to occur synchronously on the request thread. This is useful
	 * in restricted environments where programs are not permitted to start or
	 * control threads. It might also lead to better performance for programs
	 * that have extremely lightweight/trivial fitness evaluations.
	 * 
