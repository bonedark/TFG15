...
public class StatPanel extends JPanel {
	 ...
	 * INFO PROVIDER
	 */
	private CoreInfoProvider infoProvider;
	...
	private Vector<String> columnIdentifierV4;

	// store which place also include in marking in which 
	// move on log occur
	private boolean[] involvedPlaceFlags;

	
	public StatPanel(CoreInfoProvider infoProvider) {
		...
		int counter = 1;
		for (TransitionP t : infoProvider.getTransArray()) {
			items[counter++] = t;
		}
		for (PlaceP p : infoProvider.getPlaceArray()) {
		...
		// add GUI
		double[][] size = new double[][] { { 120, 400 }, { 30, 320, 30 } };
	...
	private CoreInfoProvider getInfoProvider() {
		return infoProvider;
	}

	private void showTransStats(int selectedIndex) {
	...
	private void showPlaceStats(int selectedIndex) {
		// reset involved place flags
		Arrays.fill(involvedPlaceFlags, false);
		...
		PlaceP[] places = getInfoProvider().getPlaceArray();
		XEventClass[] evClass = getInfoProvider().getEvClassArray();
		...
		for (int i : infoNumber) {
			...
			short[] infoMarking = getInfoProvider().getInfoMarking(i);
	 ...
	 * @param t
	 */
	public void setTransition(TransitionP t) {
		selection.setSelectedIndex(getInfoProvider().getIndexOf(t) + 1);
	}

	public void setPlace(PlaceP p) {
	...
	public void setInfoProvider(CoreInfoProvider infoProvider) {
		this.infoProvider = infoProvider;
		updateStats();
	}
}
