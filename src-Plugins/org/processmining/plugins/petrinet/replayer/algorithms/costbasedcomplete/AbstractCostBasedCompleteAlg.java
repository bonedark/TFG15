...
public abstract class AbstractCostBasedCompleteAlg<T extends AbstractCPNCostBasedTreeNode<T>> extends
	 ...
	 * classes (including dummy event class, i.e. an event class that does
	 * not exist in log, any transitions that are NOT silent and not mapped to
	 * any event class in the log is mapped to it) are mapped to cost; all
	 * costs should be non negative; numStates is non negative
	 */
