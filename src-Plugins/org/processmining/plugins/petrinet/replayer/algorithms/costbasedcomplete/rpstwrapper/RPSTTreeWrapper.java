...
public class RPSTTreeWrapper {
	...
	public RPSTTreeWrapper(EncPNWSetFinalMarkings codec) throws Exception {
		...
		Queue<RPSTNode<DirectedEdge, Vertex>> nodesToExpand = new LinkedList<RPSTNode<DirectedEdge, Vertex>>();
		Queue<RPSTNodeWSiblings> nodesToExpandInternal = new LinkedList<RPSTNodeWSiblings>();

		RPSTNode<DirectedEdge, Vertex> root = rpst.getRoot();
		...
		while (!nodesToExpand.isEmpty()) {
			RPSTNode<DirectedEdge, Vertex> currNode = nodesToExpand.poll();
			...
			switch (currNodeInternal.getFragmentType()) {
				...
				case TRIVIAL :
					...
					break;
				case BOND :
					Collection<RPSTEdge<DirectedEdge, Vertex>> edges2 = rpst.getOutgoingEdges(currNode);

					Set<RPSTNodeWSiblings> allChildren = new HashSet<RPSTNodeWSiblings>();

					// nothing special to do for other fragments
					for (RPSTEdge<DirectedEdge, Vertex> edge : edges2) {
						RPSTNode<DirectedEdge, Vertex> childNode = edge.getTarget();
						...
						if (childNode.getType().equals(TCType.P)) {
					...
					break;
				default : // RIGID
					Collection<RPSTEdge<DirectedEdge, Vertex>> edges3 = rpst.getOutgoingEdges(currNode);

					// nothing special to do for other fragments
					for (RPSTEdge<DirectedEdge, Vertex> edge : edges3) {
						RPSTNode<DirectedEdge, Vertex> childNode = edge.getTarget();
	...
	private void updatePolygonChildren(RPST<DirectedEdge, Vertex> rpst, RPSTNode<DirectedEdge, Vertex> currNode,
			RPSTNodeWSiblings currNodeInternal, Queue<RPSTNode<DirectedEdge, Vertex>> nodesToExpand,
		...
		assert(currNode.getType().equals(TCType.P));
		
		// children of a polygon fragment cannot be another polygon. It can only be
		// rigid, bond, or trivial
		// get children
		Collection<RPSTEdge<DirectedEdge, Vertex>> edges = rpst.getOutgoingEdges(currNode);
		int childrenSize = edges.size();

		// the children must be inserted in an ordered way
		List<RPSTNode<DirectedEdge, Vertex>> orderedVertex = getOrderedVertex(edges, currNode.getEntry());
		List<RPSTNodeWSiblings> orderedInternalChildNodes = new LinkedList<RPSTNodeWSiblings>();

		for (RPSTNode<DirectedEdge, Vertex> node : orderedVertex) {
	 ...
	 * @return
	 */
	private List<RPSTNode<DirectedEdge, Vertex>> getOrderedVertex(Collection<RPSTEdge<DirectedEdge, Vertex>> edges,
			Vertex entry) {
		List<RPSTNode<DirectedEdge, Vertex>> res = new LinkedList<RPSTNode<DirectedEdge, Vertex>>();

		Iterator<RPSTEdge<DirectedEdge, Vertex>> it = edges.iterator();
		Set<RPSTNode<DirectedEdge, Vertex>> unorderedNodes = new HashSet<RPSTNode<DirectedEdge, Vertex>>();

		RPSTNode<DirectedEdge, Vertex> lastOrderedNode = null;
		while (it.hasNext()) {
			RPSTNode<DirectedEdge, Vertex> node = it.next().getTarget();
		...
		while (!unorderedNodes.isEmpty()) {
			Iterator<RPSTNode<DirectedEdge, Vertex>> nit = unorderedNodes.iterator();
			while (nit.hasNext()) {
				RPSTNode<DirectedEdge, Vertex> node = nit.next();
