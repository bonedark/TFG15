...
public class TreeEvaluatorAStar implements FitnessEvaluator<Tree> {
	 ...
	 * @param data
	 *            Each row consists of a set of inputs and an expected output
	 *            (the last item in the row is the output).
	 */
	public TreeEvaluatorAStar(Canceller c, EvolutionLogger<Tree> evolutionLogger, XLog eventLog, double fitnessWeight,
			double simplicityWeight, double generalizationWeight, double precisionWeight) {
		this.c = c;
	...
	public double getFitness(Tree candidate, List<? extends Tree> population) {
		...
		if (fitnessWeight > 0) {
			replayFitness = calculateReplayFitness(candidate.getRoot(), Double.MAX_VALUE
			/* evolutionLogger.getWorstFitnessInLastPopulation() / log.size() */, marking2modelmove, marking2visitCount);
		...
		if (precisionWeight > 0) {
			precision = calculatePrecision(candidate.getRoot(), marking2modelmove, marking2visitCount);
		}
		...
		if (recalculateFitnessAfterReduction) {
			...
			double replayFitnessReduced = calculateReplayFitness(reducedCandidate.getRoot(),
					evolutionLogger.getWorstFitnessInLastPopulation() / log.size(), marking2modelmove,
					marking2visitCount);
			...
			if (replayFitness != replayFitnessReduced) {
				...
				System.err.println("From " + replayFitness + " to " + replayFitnessReduced);
			}/**/
		}//recalc
		 ...
		 * Simplicity
		 */
		//The overall fitness for the algorithm is replay cost + (simplicity*logsize)
		//		double simplicity = candidate.getRoot().calculateSimplicity(modelCosts(candidate.getRoot())) * log.size();
		//		simplicity++; //simplicity CAN be -1 if we calc. -1 simplicity for leafs so add 1
		...
		if (showDetails) {
			...
			System.out.println(candidate.toString());
			System.out.println("f: " + df.format(candidate.getReplayFitness()) + " p:"
					+ df.format(candidate.getPrecision()) + " s:" + df.format(candidate.getSimplicity()) + " g:"
					+ df.format(candidate.getGeneralization()));
		}

		//Remember this tree (not its fitness)

		double overallFitness = fitnessWeight * candidate.getReplayFitness() + //
		...
		//Normalize
		overallFitness /= (fitnessWeight + precisionWeight + simplicityWeight + generalizationWeight);

		candidate.setOverallFitness(overallFitness);
		return overallFitness;
	}

	private double calculateReplayFitness(Node root, double stopAt, Map<TIntList, TIntSet> marking2modelmove,
		...
		//				treeBasedAStar = new TreeBasedAStarWithoutILP(algorithm, c, root, modelCosts,
		//				marking2modelmove, marking2visitCount);
		if (root.countNodes() > 15) {
			treeBasedAStar = new TokenCountAStarWithILP(algorithm, c, root, modelCosts, marking2modelmove,
			...
			treeBasedAStar = new TokenCountAStarWithoutILP(algorithm, c, root, modelCosts, marking2modelmove,
					marking2visitCount, true);
			treeBasedAStar.setType(PerformanceType.CPUEFFICIENT);
		}
	...
	protected double calculateSimplicity(Node root) {
		...
		//Then add a punishment for unbalance which is the trees depth - the ideal depth which is log2(nrLeafs)
		//		simplicity += ((root.getDepth() - 1) - Math.ceil((Math.log(nrLeafs) / Math.log(2))));
	...
	private double calculatePrecision(Node root, Map<TIntList, TIntSet> marking2modelmove,
			TObjectIntMap<TIntList> marking2visitCount) {
