...
public class ProcessInstanceConformanceView extends JComponent implements MouseListener, MouseMotionListener {
	...
	protected static int elementWidth = 5;
	...
	protected void drawEventFlag(Graphics2D g2d, int index, int x, int y, int height) throws IndexOutOfBoundsException {
		if (nodeInstance != null){
			...
			// set color
			Color color = getColor(stepTypes.get(index));
			g2d.setColor(color);
			// draw anchor line
			...
			g2d.fillRect(x, y, width, height);
			// set color
			Color color = ProcessInstanceConformanceView.GREEN;
			g2d.setColor(color);
			// draw anchor line
			g2d.drawLine(x, y, x, y + height);
			// draw strings
	...
	private Color getColor(StepTypes stepTypes) {
		switch (stepTypes){
			case L : 
				return ProcessInstanceConformanceView.YELLOW;
			case MINVI : 
				return ProcessInstanceConformanceView.GRAY;
			case MREAL : 
				return ProcessInstanceConformanceView.PURPLE;
			case LMNOGOOD : 
				return ProcessInstanceConformanceView.RED;
			case LMGOOD : 
				return ProcessInstanceConformanceView.GREEN;
			default :
				return Color.YELLOW;
		}
	}
	...
	protected void drawEvent(Graphics2D g2d, int index, boolean active, int x, int y, int width, int height) {
		...
		if (nodeInstance != null){
			...
			// int occurrence = (eventClass != null ? eventClass.size() : 0);
			// double frequency = (maxOccurrenceCount == 0 ? 0.0 : (double) occurrence / (double) maxOccurrenceCount);
			color = ProcessInstanceConformanceView.GREEN;
		}
